...
b8 platform_startup(platform_state *plat_state, const char *application_name, i32 x, i32 y, i32 width, i32 height) {
    ...
    HWND handle =
        CreateWindowExA(window_ex_style, "vulkan_window_class", application_name,
    ...
    // Show the window
    b32 should_activate =
        1; // TODO: if the window does not accept input, this should be false.
...
LRESULT CALLBACK win32_process_message(HWND hwnd, u32 msg, WPARAM w_param, LPARAM l_param) {
    switch (msg) {
        case WM_ERASEBKGND:
            // Notify the system that we handled the background erase.
            return 1;
        case WM_CLOSE:
        ...
        case WM_DESTROY:
            ...
            return 0;
        case WM_SIZE: {
            // Get the updates size of the window.
            // RECT r;
            // GetClientRec(hwnd, &r);
